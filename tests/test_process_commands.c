...
ParameterizedTestParameters (process_commands, test_args) {
  static struct no_args_param params[] = {
    ...
    { 0x66, { 0x00 }, 1, 1, "End of sound data\n" },
    // One Nibble Arg
    { 0x70, { 0x00 }, 1, 0, "[Wait] Samples: 1\n" },
    { 0x77, { 0x00 }, 1, 0, "[Wait] Samples: 8\n" },
    { 0x7f, { 0x00 }, 1, 0, "[Wait] Samples: 16\n" },
    { 0x80, { 0x00 }, 1, 0, "[YM2612] port 0 address 2A, wait samples: 0\n" },
    { 0x87, { 0x00 }, 1, 0, "[YM2612] port 0 address 2A, wait samples: 7\n" },
    { 0x8f, { 0x00 }, 1, 0, "[YM2612] port 0 address 2A, wait samples: 15\n" },
    // One Byte Arg
    { 0x4f, { 0x01 }, 1, 0, "[Game Gear PCM] Write 0x01 to port 0x06\n" },
    { 0x50, { 0x01 }, 1, 0, "[PSG] Write 0x01\n" },
    { 0x30, { 0x01 }, 1, 0, "[reserved]\n" },
    { 0x37, { 0x01 }, 1, 0, "[reserved]\n" },
    { 0x3f, { 0x01 }, 1, 0, "[reserved]\n" },
    // Two byte args
    { 0x54, { 0x01, 0x02 }, 2, 0, "[YM2151] Write 0x01 to reg 0x02\n" },
    { 0x67, { 0xff, 0x01, 0x00, 0x00, 0x00, 0xff }, 6, 0, "[Data Block] Data Type 0xff, Size 0x01\n"  },
  };
...
ParameterizedTest (struct no_args_param *params, process_commands, test_args) {
  ...
  cr_assert_eq(vgm_process_command(g_mem_stream, &cmd), params->ret_value);
  fflush(g_mem_stream);
