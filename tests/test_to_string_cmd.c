...
ParameterizedTestParameters(stringify, test_to_string) {
  static struct to_string_param_t params[] = {
    // No args
    {0x62, cmd_type_none, action_type_wait, {0x00}, 1, "[Wait] One frame (60Hz)"},
    {0x63, cmd_type_none, action_type_wait, {0x00}, 1, "[Wait] One frame (50Hz)"},
    {0x66, cmd_type_none, action_type_eos, {0x00}, 1, "[EOS] End of sound data"},
    // One Nibble Arg
    {0x70, cmd_type_nibble_inc, action_type_wait, {0x00}, 1, "[Wait] Samples: 1"},
    {0x77, cmd_type_nibble_inc, action_type_wait, {0x00}, 1, "[Wait] Samples: 8"},
    {0x7f, cmd_type_nibble_inc, action_type_wait, {0x00}, 1, "[Wait] Samples: 16"},
    {0x80, cmd_type_nibble, action_type_ym2612,
     {0x00},
     1,
     "[YM2612] [0x2a]:0 <- data bank, then wait 0 samples"},
    {0x87, cmd_type_nibble, action_type_ym2612,
     {0x00},
     1,
     "[YM2612] [0x2a]:0 <- data bank, then wait 7 samples"},
    {0x8f, cmd_type_nibble, action_type_ym2612,
     {0x00},
     1,
     "[YM2612] [0x2a]:0 <- data bank, then wait 15 samples"},
    // One Byte Arg
    {0x4f, cmd_type_byte, action_type_game_gear_pcm,  {0x01}, 1, "[Game Gear PCM] :6 <- 0x01"},
    ...
    // Two byte args
    {0x54, cmd_type_byte_byte, action_type_ym2151, {0x01, 0x02}, 2, "[YM2151] [0x01] <- 0x02"},
    {0x67, cmd_type_byte_byte, action_type_data_block,
...
ParameterizedTest(struct to_string_param_t *params, stringify,
                  test_to_string) {
  struct vgm_command_t cmd = {params->cmd, params->cmd_type, params->action, params->data, params->data_size};
  char str[512];
